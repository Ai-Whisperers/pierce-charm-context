# Cuestionario 09 — Inventario inicial de joyería

> **Día 9.** Cuando el cliente compre las primeras piezas, completar este cuestionario.
> **Bloque:** SKU, fotos, precios, materiales.
> **Owner:** Luana (cuando reciba stock).

---

## 1. Compra inicial

1.1. **¿Ya compraron la primera tanda de joyería?** (sí/no + cuándo)
_[respuesta]_

1.2. **¿Cuántas piezas en el primer pedido?** (total aproximado)
_[respuesta]_

1.3. **¿Cuántas marcas diferentes?**
_[respuesta]_

1.4. **¿Cuál es el proveedor / tienda?**
_[respuesta]_

## 2. SKU y naming

Para cada pieza, necesitamos:
- **SKU interno** (ej. J001, J002)
- **Nombre visible al cliente** (ej. "Septum clicker calavera")
- **Categoría** (Cadenas / Septum & Nariz / Captives / etc.)
- **Material** (titanio / acero / oro / otro)
- **Color** (plateado / negro PVD / dorado / tornasol / rojo-rosado)
- **Calibre / diámetro** (ej. 1.2mm, 6mm ø)
- **Precio de venta** (Gs)

2.1. **Cargar tabla de inventario:**

| SKU | Nombre visible | Categoría | Material | Color | Calibre/ø | Precio Venta | Foto |
|---|---|---|---|---|---|---|---|
| J001 | | | | | | | |
| J002 | | | | | | | |
| ... | | | | | | | |

## 3. Fotos por pieza

3.1. **¿Tomaron foto de cada pieza con fondo limpio / terciopelo?**
_[respuesta]_

3.2. **Cada foto debe estar mínimo 1080×1080 px** (o específico para Instagram / web).
_[OK]_

3.3. **¿Las suben a `assets/photos/joyeria/` con SKU como nombre de archivo?**
_(Sugerencia: `J001.jpg`, `J002.jpg`)_.
_[respuesta]_

## 4. Materiales certificados

4.1. **¿Las piezas de titanio vienen con certificado de la marca?** (sí/no)
_[respuesta]_

4.2. **¿Tienen las marcas certificadas documentadas en algún lugar?**
_[respuesta]_

## 5. Catálogo PDF

5.1. **¿Ya tienen el catálogo diseñado por la amiga de Luana?**
_[respuesta]_

5.2. **¿Lo pueden subir al repo?**
_(Sugerencia: `assets/catalogos/joyeria-v1.pdf`)_
_[respuesta]_

---

## Próximos pasos

Erebus reemplaza los 12 placeholders de /galeria con fotos reales + nuevas piezas. Si hay 20+ piezas, agregar filtros (material+color+zona).


---

# Respuestas — Cuestionario 09 ✅ 2026-07-07 (audio 2)

**Q01:** Todavía no compraron la primera tanda.
**Q02:** Estimada **50-100 piezas**, distintos tamaños (tiene duda sobre el número).
**Q03:** Tipos: **labrets, industrial, septum, circulares** — cubiertos.
**Q04:** Proveedor actual: **AliExpress (chinos)** en general.
**Q05-06:** Una pieza por SKU = nombre visible + SKU interno + material + color + calibre + precio.
**Q07-08:** Foto por pieza cuando llegue inventario.
**Q09-10:** **Certificados de titanio:** SÍ va a fotografiar los certificados de cada proveedor y subir foto al sitio.
**Q11-12:** Catálogo PDF de referencia: tiene el catálogo diseñado de **Dark Charm** (su otra tienda de ropa alternativa + accesorios) → usar como inspiración para el catálogo de joyería piercing.

## Pendientes
- Iván envía foto de las primeras 50-100 piezas cuando lleguen.
- Cada foto con fondo limpio / terciopelo, nombre archivo = SKU (J001.jpg).
- Subir a `assets/photos/joyeria/` en el repo de contexto.
- Cuando llega, Erebus:
  - Reemplaza los 12 placeholders SVG en /galeria con fotos reales.
  - Si hay 20+ piezas, agrega filtros (material + color + zona).
  - Sube certificados del proveedor al sitio en /legal/certificados.
- Luana envía el catálogo Dark Charm para inspiración visual cuando lo comparta.